...
{
	_effect = gs::effect::create(streamfx::data_file_path("effects/displace.effect").string());
...
void displacement_factory::get_defaults2(obs_data_t* data)
{
	obs_data_set_default_string(data, ST_FILE,
								streamfx::data_file_path("examples/normal-maps/neutral.png").string().c_str());
...
obs_properties_t* displacement_factory::get_properties2(displacement_instance* data)
{
	...
	if (data) {
		...
		path = streamfx::data_file_path("examples/normal-maps/neutral.png").string();
	}
